...
# ---------------- Display Projects ----------------
cols = st.columns(5)  # 5 columns for 5 projects

for i, proj in enumerate(projects):
    with cols[i]:
        image_path = os.path.join(".", proj["image"])  # Ensure images are in same folder as app.py
        if os.path.exists(image_path):
            st.image(image_path, width=200)
        else:
            st.error(f"Image not found: {proj['image']}")
